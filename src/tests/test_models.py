...
class TestModelsClass(TestBaseClass):
    ...
    def test_card_no_validation_check(self):
        """
            ...
            테스트 합니다.
        """
        mixer.blend(User, card_no=self.valid_card_no1, username='Daniel', limitation=10000, balance=0)
    ...
    def test_transaction_models_credit_case_without_transaction_zone(self):
        ...
        chris = User.query.filter_by(card_no=self.valid_card_no2, username=self.test_username2).first()
        ...
        tr = mixer.blend(
                ...
                type=1,
                tram=1000,
                label=f'{chris.username} Credit {amount}'
            )
        ...
        assert chris.balance == amount, "1000원을 Credit 했기 때문에 +1000원 이여야합니다."
        assert tr.tram == amount, f"{amount}, 값만큼 트렌젝션이 발생해야합니다."
    ...
    def test_all_transaction_model_test_with_bdd_story_lines(self):
        ...
        mixer.blend(
            ...
            type=1,
            tram=5000,
            label=f'{trump_user_a.username} Credit {amount}'
        )
        ...
        trump_user_b = User.query.filter_by(card_no=self.valid_card_no3, username=self.test_username3).first()
        ...
        mixer.blend(
            ...
            type=0,
            tram=next_amount,
            # label=f'{trump_user_b.username} Credit {amount}'
            label=f'{trump_user_b.username} Charged {next_amount} at the Sumgo Shop'
        )
        ...
        try:
            ...
            trump_user_c = User.query.filter_by(card_no=self.valid_card_no3, username=self.test_username3).first()
            ...
            mixer.blend(
                ...
                type=0,
                tram=next_amount,
                label=f'{trump_user_c.username} Charged {after_next_amount} at the Sumgo Shop'
            )
